...
```
Why are some of these rhombuses darker than others? Transparency (`alpha`) allows us to see when multiple points are overlaid on one another, i.e. there are multiple datapoints at the locations of the less transparent rhombuses.
...
```
Note that here, we only specify 'color'; this is because the default point shape in R doesn't have a fill parameter.
...
ggplot(data = iris) +
  geom_point(
    mapping = 
      aes(x = Sepal.Length, y = Sepal.Width, shape = Species, size = Petal.Width),
    color = 'red')
```
...
ggplot(data = iris) +
  geom_density(mapping = aes(x = Petal.Length)) +
  geom_density(mapping = aes(x = Petal.Length, color = Species))
...
ggplot(data = iris) +
  geom_density(mapping = aes(x = Petal.Length)) +
  geom_density(mapping = aes(x = Petal.Length, color = Species), linetype = 2)
```
...
Add animations to plots (seems like it doesn't animate in the html notebook)
